...
+ Python Library to provide a selection menu as an interface with the WCPS server that provides a list of WCS/WCPS operations and functions
  - A selection menu with a list of menu choices to showcase and execute the WCS and WCPS operations.[start_menu.py](src/wdc/start_menu.py) 
    ...
    - WCS request in a python code with subsetting by slicing on time axis and trimming on spatial axes, and encoding the output into image jpeg [opt2_wcs_subsetting.py](src/wdc/opt2_wcs_subsetting.py)
    ...
    - WCPS query to retrieve the monthly average temperature of Bremen City in 2014 and plot it into a graph [opt7_wcps_mthly_average.py]
    - Quit [opt8_quit]
  
 + Implementation Schedule and Plan
   - Sprint#1(8 April to 18 April , 2 weeks): Define project scope, project plan, project design and setup the environment with some sample functions, source codes and test cases
...
+ Prerequisites
  ...
  - Install required Python Libraries
    ```
    $pip install requests Image owslib numpy scipy matplotlib
    ```   
+ Packaging and installation (not started yet)
+ Start running the project: run start_menu.py
  ```
  start_menu.py
  ```
...
+ Integration Test:
  - Path: [sprint_1/test_case/Integration_test](test_case/Integration_test)
...
+ Work not started yet:
  - operations and functions not started yet:
    ...
    - make the query parameters as input variables to provide more dynamic query requests, including
      ...
      - from and to date of a time period in retrieving the temperature data
